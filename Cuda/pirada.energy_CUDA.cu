 ...
 */
 ...
 __global__ void cMax(float *layer, float *maximos, int *posiciones, float* maximosTemp, int layer_size, int i){
    ...
    for (int s=1; s < blockDim.x; s*=2){
        ...
        if(new_lid < blockDim.x){
            // if ( layer[k] > layer[k-1] && layer[k] > layer[k+1] ) {
                if ( sdata[new_lid*2] < sdata[(new_lid+s)*2] ) {
                    sdata[new_lid*2] = sdata[(new_lid+s)*2]; // Copy max
                    sdata[new_lid*2+1] = sdata[(new_lid+s)*2+1]; // Copy pos
                }
            // }
        }
        __syncthreads();
    ...
    if(lid == 0){
        maximosTemp[blockIdx.x*2] = sdata[0];
        maximosTemp[blockIdx.x*2+1] = sdata[1];
    }
    ...
    if(gid == 0){
        ...
        for(int j = 1; j < gridDim.x; j++) {
            ...
            if (maximos[i] < maximosTemp[j*2]){
                maximos[i] = maximosTemp[j*2];
                posiciones[i] = (int) maximosTemp[j*2+1];
            }
        }
        // printf("\n");
    }
 ...
 int main(int argc, char *argv[]) {
     ...
     cudaMalloc((void**)&dMaximosTemp,2*sizeof(float) * gridShapeGpuFunc1.x);
     ...
     for( i=0; i<num_storms; i++) {
        ...
        cMax<<<gridShapeGpuFunc1,bloqShapeGpuFunc1, 256*2*sizeof(float)>>>(dlayer, dMaximos, dPosiciones, dMaximosTemp, layer_size, i);
         cudaDeviceSynchronize();
     }
